...
- 過去のPapyrusによるリアクションと返信は集計から除外する。既存DBとの互換性のため、
  旧Papyrus・人間用の重みと返信の根拠を保持する列は残すが、設定・集計には使用しない。
...
- チャンネル単位の除外は `cynicism_excluded_channels` に環境別に保存する。リアクションの記録は続け、
  集計時に分子・分母・発言一覧・最多ポイントの発言へ同じ除外条件を適用する。
  親チャンネルからスレッドへの継承はせず、解除すれば記録済みの過去分も集計に戻る。
- 保存済みメッセージが別のBotによる投稿だった場合は、配送記録を上書きせず設定エラーとして停止する。
