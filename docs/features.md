...
- **長期記憶文書**: 人物別、Papyrus自身、サーバー共有のMarkdown文書を会話単位で更新します。
  未処理の会話が50メッセージ、4,000トークン、または最後の人間投稿から12時間のいずれかに達すると
  バックグラウンドで分析します。分析時には未処理部分を最大4,000トークンとし、直前の参照会話を含めて
  最大5,000トークンを渡します。応答時には短期会話に登場・言及された人物の文書と、Papyrus・共有文書を
  参照します。Papyrus自身の通常の発言も、Papyrusの立場や意見を更新する根拠として扱います。
...
- **冷笑王の発表**: 週、月、年ごとに指定チャンネルへ発表します。週は金曜22時(JST)で区切り、
  ...
  集計期間はJST暦日で区切ります(API利用レポートのUTC暦日とは基準が異なります)。
