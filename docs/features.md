...
- **添付・Embed解析**: Chatbotの文脈に含まれる画像、PDF、Discordが展開したEmbedの情報と画像を解析し、
  応答へ利用します。
- **短期記憶**: チャンネルまたはスレッドごとに直近最大5,000トークンを保持します。最後の人間投稿から
  12時間空いた後の次の人間投稿では、新しい会話として短期記憶を区切ります。
- **長期記憶文書**: 人物別、Papyrus自身、サーバー共有のMarkdown文書を会話単位で更新します。
  ...
  agree／disagree、Spotify Embed補完、予定イベント通知、監査ログ転送など、機能が自動生成した投稿は分析対象外です。
