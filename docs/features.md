...
期間には `weekly`、`monthly`、`yearly` を指定します。開始日は期間内の任意の日でよく、省略すると
直近の完了済み期間を対象にします。`publish`、`weight`、`pause`、`resume` はBot管理者のみ実行できます。
...
- **冷笑王の発表**: 週(月曜始まり)、月、年ごとに、期間終了の翌日21時(JST)へ指定チャンネルへ発表します。
